...
async fn create_unconfirmed_subscriber(app: &TestApp) -> ConfirmationLinks {
    // We are working with multiple subscribers now, their details must be
    // randomized to avoid conflicts.
...
async fn newsletter_creation_is_idempotent() {
    ...
    // Act 2 - Follow the redirect and verify.
    let html_page = app.get_publish_newsletter_html().await;
    assert!(html_page.contains(
        "<p><i>The newsletter issue has been accepted. Emails will go out shortly.</i></p>"
    ));

    // Act 3 - Submit a newsletter form again.
    let response = app.post_newsletter(&newsletter_request_body).await;
    assert_is_redirect_to(&response, "/admin/newsletters");

    // Act 4 - Follow the redirect and verify.
    let html_page = app.get_publish_newsletter_html().await;
    assert!(html_page.contains(
        "<p><i>The newsletter issue has been accepted. Emails will go out shortly.</i></p>"
    ));
