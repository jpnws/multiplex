...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    // based on the value TEST_LOG because the sink is part of the
    // type returned by `get_subscriber`. Therefore, they are not
    // the same type. We could work around it, but this is the most
...
async fn spawn_app() -> TestApp {
    ...
    // All other invocations will stead skip execution.
    Lazy::force(&TRACING);
